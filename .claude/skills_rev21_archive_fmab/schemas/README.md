...
## decision_record and portfolio (added, revision 2.1)

`decision_record` is written by O1 for every routed closure: what launched, what was carded, what was refused and why, the guard state and the envelope after commitment. `portfolio` holds the live candidates from the D4 alternatives with their priority, and the router's guard state per candidate. `scripts/router.py` writes both.
